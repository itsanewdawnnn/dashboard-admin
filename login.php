<?php
require_once __DIR__ . '/includes/auth.php';

ensureSessionStarted();

if (isLoggedIn()) {
    header('Location: index.php');
    exit();
}

$errorMessage = '';

// Jika login gagal, user tidak perlu mengetik ulang username.
$username = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $submittedCsrfToken = null;
    if (isset($_POST['csrf_token'])) {
        $submittedCsrfToken = $_POST['csrf_token'];
    }

    if (isset($_POST['username'])) {
        $username = trim($_POST['username']);
    }

    $password = '';
    if (isset($_POST['password'])) {
        $password = (string) $_POST['password'];
    }

    $rememberMe = isset($_POST['remember']);

    if (isLoginLocked()) {
        $errorMessage = 'Too many failed login attempts. Please try again in a few minutes.';
    } elseif (!verifyCsrfToken($submittedCsrfToken)) {
        $errorMessage = 'Invalid form session (CSRF). Please reload the page and try again.';
    } elseif ($username === '' || $password === '') {
        $errorMessage = 'Username and password are required.';
    } else {
        $user = attemptLogin($username, $password);

        if ($user) {
            loginUser($user, $rememberMe);
            header('Location: index.php');
            exit();
        }

        registerFailedLogin();
        $errorMessage = 'Invalid username or password.';
    }
}

$csrfToken = csrfToken();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Mazer Admin Dashboard</title>
    <link rel="shortcut icon" href="data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2033%2034'%20fill-rule='evenodd'%20stroke-linejoin='round'%20stroke-miterlimit='2'%20xmlns:v='https://vecta.io/nano'%3e%3cpath%20d='M3%2027.472c0%204.409%206.18%205.552%2013.5%205.552%207.281%200%2013.5-1.103%2013.5-5.513s-6.179-5.552-13.5-5.552c-7.281%200-13.5%201.103-13.5%205.513z'%20fill='%23435ebe'%20fill-rule='nonzero'/%3e%3ccircle%20cx='16.5'%20cy='8.8'%20r='8.8'%20fill='%2341bbdd'/%3e%3c/svg%3e" type="image/x-icon">
    <link rel="shortcut icon" href="data:image/png;base64," type="image/png">
  <link rel="stylesheet" href="assets/compiled/css/app.css">
  <link rel="stylesheet" href="assets/compiled/css/app-dark.css">
  <link rel="stylesheet" href="assets/compiled/css/auth.css">
  <style>
    /* Tata letak login ringkas & terpusat, menggantikan layout besar rata-kiri bawaan Mazer. */
    #auth-left {
        display: flex;
        flex-direction: column;
        height: 100%;
        padding: 3rem 4rem;
    }
    .auth-form-area {
        /* flex-start, bukan center -- center akan menyeimbangkan ulang lewat margin-bottom logo. */
        flex: 1 1 auto;
        display: flex;
        align-items: flex-start;
        justify-content: center;
    }
    .auth-form-wrap {
        width: 100%;
        max-width: 22rem;
    }
    .auth-logo-compact {
        text-align: center;
        margin-bottom: 10rem;
    }
    .auth-logo-compact img {
        height: 3rem;
    }
    .auth-title-compact {
        font-size: 2.1rem;
        font-weight: 700;
        text-align: center;
        margin-bottom: 2rem;
        color: var(--bs-body-color);
    }
    .auth-submit-btn {
        padding: .6rem 2.25rem;
        font-weight: 600;
        border-radius: .5rem;
    }
    .auth-footer {
        flex-shrink: 0;
        text-align: center;
        padding-top: 1rem;
        font-size: .85rem;
        color: var(--bs-secondary-color);
    }

    #auth-right {
        /* Lebih gelap dari gradien bawaan Mazer agar teks sekunder tetap kontras. */
        height: 100%;
        display: flex;
        align-items: flex-start;
        justify-content: center;
        padding-top: 18vh;
    }
    /* 2 ID selector diperlukan agar mengalahkan #auth #auth-right di auth.css (specificity). */
    #auth #auth-right,
    html[data-bs-theme=dark] #auth #auth-right {
        background: url(assets/compiled/png/4853433.png), linear-gradient(90deg, #24397a, #33488c);
    }
    .auth-right-content {
        text-align: center;
        padding: 0 2rem;
    }
    .auth-right-title {
        /* Warna eksplisit -- selector h1 bawaan Mazer mengalahkan warna putih yang diwariskan. */
        font-size: 2.5rem;
        font-weight: 700;
        margin-bottom: 1rem;
        color: #fff;
    }
    .auth-right-subtitle {
        font-size: 1.25rem;
        font-weight: 600;
        line-height: 1.6;
        margin-bottom: 0;
        color: rgba(255, 255, 255, .6);
    }

    /* Tombol show/hide password -- teknik sama dengan halaman My Profile. */
    .password-input-wrapper {
        position: relative;
    }
    .password-input-wrapper .form-control {
        padding-right: 2.75rem;
    }
    .password-toggle-btn {
        position: absolute;
        top: 50%;
        right: .75rem;
        transform: translateY(-50%);
        display: flex;
        align-items: center;
        justify-content: center;
        width: 2rem;
        height: 2rem;
        background: none;
        border: none;
        border-radius: 50%;
        padding: 0;
        line-height: 1;
        color: #a6a8aa;
        cursor: pointer;
        transition: background-color .15s ease, color .15s ease;
    }
    .password-toggle-btn:hover,
    .password-toggle-btn:focus-visible {
        color: #5a8dee;
        background-color: rgba(90, 141, 238, .1);
    }
    .password-toggle-btn i {
        font-size: 1.1rem;
    }
  </style>
</head>

<body>
    <script src="assets/static/js/initTheme.js"></script>
    <div id="auth">
        <div class="row h-100 g-0">
            <div class="col-lg-5 col-12">
                <div id="auth-left">
                    <div class="auth-form-area">
                        <div class="auth-form-wrap">
                            <div class="auth-logo-compact">
                                <a href="login.php"><img src="data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%20152%2034'%20fill-rule='evenodd'%20stroke-linejoin='round'%20stroke-miterlimit='2'%3e%3cpath%20d='M0%2027.472c0%204.409%206.18%205.552%2013.5%205.552%207.281%200%2013.5-1.103%2013.5-5.513s-6.179-5.552-13.5-5.552c-7.281%200-13.5%201.103-13.5%205.513z'%20fill='%23435ebe'%20fill-rule='nonzero'/%3e%3ccircle%20cx='13.5'%20cy='8.8'%20r='8.8'%20fill='%2341bbdd'/%3e%3cpath%20d='M71.676%203.22c.709%200%201.279.228%201.71.684.431.431.646%201.013.646%201.748v22.496c0%20.709-.203%201.267-.608%201.672s-.937.608-1.596.608-1.178-.203-1.558-.608-.57-.963-.57-1.672V12.492l-6.46%2012.236c-.304.557-.633.975-.988%201.254-.355.253-.773.38-1.254.38s-.899-.127-1.254-.38-.684-.671-.988-1.254l-6.498-12.046v15.466c0%20.684-.203%201.241-.608%201.672-.38.405-.899.608-1.558.608s-1.178-.203-1.558-.608-.57-.963-.57-1.672V5.652c0-.735.203-1.317.608-1.748.431-.456%201.001-.684%201.71-.684.988%200%201.761.545%202.318%201.634l8.436%2016.074%208.398-16.074c.557-1.089%201.305-1.634%202.242-1.634zm15.801%207.942c2.584%200%204.497.646%205.738%201.938%201.267%201.267%201.9%203.205%201.9%205.814v9.272c0%20.684-.203%201.229-.608%201.634-.405.38-.962.57-1.672.57-.658%200-1.203-.203-1.634-.608-.405-.405-.608-.937-.608-1.596v-.836c-.431.988-1.114%201.761-2.052%202.318-.912.557-1.976.836-3.192.836-1.241%200-2.368-.253-3.382-.76s-1.811-1.203-2.394-2.09-.874-1.875-.874-2.964c0-1.368.342-2.445%201.026-3.23.71-.785%201.85-1.355%203.42-1.71s3.737-.532%206.498-.532h.95v-.874c0-1.241-.266-2.141-.798-2.698-.532-.583-1.393-.874-2.584-.874a7.78%207.78%200%200%200-2.242.342c-.76.203-1.659.507-2.698.912-.658.329-1.14.494-1.444.494-.456%200-.836-.165-1.14-.494-.278-.329-.418-.76-.418-1.292%200-.431.102-.798.304-1.102.228-.329.596-.633%201.102-.912.887-.481%201.938-.861%203.154-1.14%201.242-.279%202.458-.418%203.648-.418zm-1.178%2015.922c1.267%200%202.293-.418%203.078-1.254.811-.861%201.216-1.963%201.216-3.306v-.798h-.684c-1.697%200-3.015.076-3.952.228s-1.608.418-2.014.798-.608.899-.608%201.558c0%20.811.279%201.482.836%202.014.583.507%201.292.76%202.128.76zm27.476-.456c1.418%200%202.128.595%202.128%201.786%200%20.557-.178%201.001-.532%201.33-.355.304-.887.456-1.596.456h-12.692c-.634%200-1.153-.203-1.558-.608a1.97%201.97%200%200%201-.608-1.444c0-.583.228-1.14.684-1.672l9.766-11.286h-8.474c-.71%200-1.242-.152-1.596-.456s-.532-.747-.532-1.33.177-1.026.532-1.33.886-.456%201.596-.456h12.274c.658%200%201.178.203%201.558.608.405.38.608.861.608%201.444%200%20.608-.216%201.165-.646%201.672l-9.804%2011.286h8.892zm19.762-1.52c.431%200%20.773.165%201.026.494.279.329.418.773.418%201.33%200%20.785-.468%201.444-1.406%201.976-.861.481-1.836.874-2.926%201.178-1.089.279-2.128.418-3.116.418-2.989%200-5.358-.861-7.106-2.584s-2.622-4.079-2.622-7.068c0-1.9.38-3.585%201.14-5.054s1.824-2.609%203.192-3.42c1.394-.811%202.964-1.216%204.712-1.216%201.672%200%203.129.367%204.37%201.102s2.204%201.773%202.888%203.116%201.026%202.926%201.026%204.75c0%201.089-.481%201.634-1.444%201.634h-11.21c.152%201.748.646%203.04%201.482%203.876.836.811%202.052%201.216%203.648%201.216.811%200%201.52-.101%202.128-.304.634-.203%201.343-.481%202.128-.836.76-.405%201.318-.608%201.672-.608zm-6.574-10.602c-1.292%200-2.33.405-3.116%201.216-.76.811-1.216%201.976-1.368%203.496h8.588c-.05-1.545-.43-2.711-1.14-3.496-.709-.811-1.697-1.216-2.964-1.216zm22.43-3.268c.658-.051%201.178.089%201.558.418s.57.823.57%201.482c0%20.684-.165%201.191-.494%201.52s-.925.545-1.786.646l-1.14.114c-1.495.152-2.597.659-3.306%201.52-.684.861-1.026%201.938-1.026%203.23v7.98c0%20.735-.228%201.305-.684%201.71-.456.38-1.026.57-1.71.57s-1.254-.19-1.71-.57c-.431-.405-.646-.975-.646-1.71V13.442c0-.709.215-1.254.646-1.634.456-.38%201.013-.57%201.672-.57s1.19.19%201.596.57c.405.355.608.874.608%201.558v1.52c.481-1.115%201.19-1.976%202.128-2.584.962-.608%202.026-.95%203.192-1.026l.532-.038z'%20fill='%23435ebe'%20fill-rule='nonzero'/%3e%3c/svg%3e" alt="Logo"></a>
                            </div>
                            <h1 class="auth-title-compact">Login</h1>

                            <?php if ($errorMessage !== ''): ?>
                            <div class="alert alert-danger" role="alert">
                                <?= htmlspecialchars($errorMessage, ENT_QUOTES, 'UTF-8') ?>
                            </div>
                            <?php endif; ?>

                            <form method="post" action="login.php" autocomplete="off">
                                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>">
                                <div class="form-group mb-3">
                                    <label for="username" class="form-label">Username</label>
                                    <input type="text" name="username" id="username" class="form-control" required autofocus value="<?= htmlspecialchars($username, ENT_QUOTES, 'UTF-8') ?>">
                                </div>
                                <div class="form-group mb-3">
                                    <label for="login_password" class="form-label">Password</label>
                                    <div class="password-input-wrapper">
                                        <input type="password" name="password" id="login_password" class="form-control" required>
                                        <button type="button" class="password-toggle-btn" id="toggleLoginPassword" aria-label="Show password">
                                            <i class="bi bi-eye" id="toggleLoginPasswordIcon"></i>
                                        </button>
                                    </div>
                                </div>
                                <div class="form-check d-flex align-items-center mb-4">
                                    <input class="form-check-input me-2" type="checkbox" value="1" name="remember" id="flexCheckDefault">
                                    <label class="form-check-label text-gray-600" for="flexCheckDefault">
                                        Keep me logged in
                                    </label>
                                </div>
                                <div class="text-center">
                                    <button class="btn btn-primary auth-submit-btn">Log In</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    <footer class="auth-footer">
                        <?= date('Y') ?> &copy; Itsanewdawnnn
                    </footer>
                </div>
            </div>
            <div class="col-lg-7 d-none d-lg-block">
                <div id="auth-right">
                    <div class="auth-right-content">
                        <h1 class="auth-right-title">Admin Dashboard</h1>
                        <p class="auth-right-subtitle">Mazer<br>Itsanewdawnnn</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script>
        document.getElementById('toggleLoginPassword').addEventListener('click', function () {
            const input = document.getElementById('login_password');
            const icon = document.getElementById('toggleLoginPasswordIcon');
            const isHidden = input.getAttribute('type') === 'password';

            if (isHidden) {
                input.setAttribute('type', 'text');
                icon.classList.remove('bi-eye');
                icon.classList.add('bi-eye-slash');
            } else {
                input.setAttribute('type', 'password');
                icon.classList.remove('bi-eye-slash');
                icon.classList.add('bi-eye');
            }
        });
    </script>
</body>
</html>